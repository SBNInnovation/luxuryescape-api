...
const addCustomizeQuote = async (req: Request, res: Response): Promise<void> => {
    try {
        const { name, email, number, message, tourName } = req.body;

        if (!name || !email || !number || !message || !tourName) {
            res.status(400).json({ success: false, message: "Please fill all fields." });
            return;
        }

        const quote = await CustomizeQuote.create({ name, email, number, message, tourName });

        if (!quote) {
            res.status(400).json({ success: false, message: "Failed to add customize quote." });
            return;
        }
        ...
        const mailOptions = {
            ...
            to: process.env.EMAIL, // Admin receives the email
            replyTo: email, // Admin can reply to the client's email
            ...
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border-radius: 10px; background-color: #ffffff; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); border-left: 6px solid #E8B86D;">
                    ...
                    <p style="font-size: 18px; color: #444; line-height: 1.6;"><strong>Tour:</strong> ${tourName}</p>
