...
import express from "express";
import addTourTypes, { MulterRequest } from "../../controllers/tourTypes/addTourTypes.controller.js";
import multer from "multer";
import fs from "fs";
import path from "path";
...
// Define the route to handle tour type upload
addTourTypesRouter.post(
  "/tour/add-tour-type",
  uploader.single("thumbnail"), // Handle single file upload for 'thumbnail'
  (req, res) => {
    addTourTypes(req as MulterRequest, res); // Explicit type assertion for the request
  }
);
